...
```{r}
library(tidyverse)
#library(ggplot2) This is not necessary, ggplot2 is included in tidyverse
...
```
We know there is a correlation between G1, G2, and G3. How do previous grades affect a student's decision to drop?
```{r}
drop %>%
  filter(drop == TRUE) %>%
  ggplot(aes(G1)) +
  ...
  ylab("Count") +
  ggtitle("First period grades among dropped students")

drop %>%
  filter(drop == TRUE & G2 == 0) %>%
  ggplot(aes(G1)) +
  geom_bar() +
  xlab("First period grade in percentage") +
  ylab("Count") +
  ggtitle("First period grades among students who dropped during the second period")
```
...
drop %>%
  ...
  xlab("First period grade in percentage") +
  ylab("Count") +
  ggtitle("First period grades among students who did not drop")

drop %>%
  filter(drop==FALSE) %>%
  summarize(mean=mean(G1))
```
...
__ANALYSIS OF OTHER SIGNIFICANT FACTORS FOUND IN PREVIOUS PARTS OF REPORT__