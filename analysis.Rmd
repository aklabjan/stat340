...
```
The coloring in the plot was determined by if the G3 column in our data set was equal to zero. The red points represent students who dropped out of the class before the end of the year. This trend lead us to the focus of our report: What variables have an effect on weather a student will drop out?
```{r}
drop <- data %>%
...
```{r}
library(rpart)
test_inds <- sample(1:nrow(drop2), nrow(drop2)/5);
drop.test <- drop2[test_inds,];
drop.train <- drop2[-test_inds,];
rt <- rpart(dropped~., data=drop.train, method='anova')
plot(rt, uniform=TRUE, main='Regression tree for Droppping Out')
text(rt, all=TRUE, use.n = TRUE, cex=.8 )

rt
```
```{r}
yhat_test <-predict( rt,drop.test);
test_resids <- yhat_test - drop.test$dropped; # compute the residuals
